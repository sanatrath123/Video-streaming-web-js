const apikey = "0da844c3a633540ac068fad5c3af0928"
...
const apiPaths ={
  ...
  searchOnYoutube: (query) => `https://www.googleapis.com/youtube/v3/search?part=snippet&q=${query}&key=AIzaSyCx8JrUVfYLjVznfk-c_XnQaVlx_TFUdY4`
}
//fetch trending movies
function Trending_section(){
    fetchAndbuildMovieSection(apiPaths.fetchTrending,'Trending Now')
    .then(list=>{
            const randomIndex = parseInt(Math.random()*list.length);
            buildBannnerSection(list[randomIndex])
            .catch(res=>{
                console.log(err);
            });
        })
...
function buildBannnerSection(movie)
{
...
function fetchAndbuildMovieSection(fetchUrl, categoryName){
 ...
 function buildMoviesSection(list, categoryName) {
    ...
    const moviesSectionHTML = `
        <div class="movies-section" id="movies-allign">
...
//call the init after a page load
